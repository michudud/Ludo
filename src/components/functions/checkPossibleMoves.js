const checkPossibleMoves = (player, otherPlayers, setPlayerState) => {
      ...
      if (typeof player[currPawn] === "string") {
        if (player[currPawn].includes("s")) {
          availableMoves.push({
            ...
            pawnScoreAdd: 1,
            executeMove: () => {
              setPlayerState[player.color]({
                ...player,
                [currPawn]: player.startPos,
              });
            },
          });
        }
          ...
          if (player[currPawn] === player.startPos) {
            availableMoves.push({
              ...
              executeMove: () => {
                setPlayerState[player.color]({
                  ...player,
                  [currPawn]: player[currPawn] + diceResult,
                });
              },
            });
          } else {
          ...
          if (player[currPawn].includes("s") && diceResult === 6) {
            if (otherPlayers[i][otherPlayerPawn] === player.startPos) {
              availableMoves.push({
                ...
                moveResults: () => {
                  setPlayerState[otherPlayers[i].color]((previousState) => ({
                    ...previousState,
                    [otherPlayerPawn]: j + otherPlayers[i].color[0] + "s",
                  }));
                },
              });
            }
